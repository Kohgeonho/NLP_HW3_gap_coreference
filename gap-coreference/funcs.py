...
def subject(row):
    ...
    ## 6. When both words are not subject
        ...
        ## 6-1. When both words appear latter than pronoun, and there is another subject rather than the words.
        if both_latter(row) and another_subject(row):
            return (False, False)

        ## 6-2. When pattern like <(word) and (his|her)> appears, coreference of the word is always true.
        elif exist_CC_pronoun(row):
            return CC_pronoun(row)

        ## 6-3 Check the frequency of words in the text.
        else:
            ## 6-3-1 If a word appears multiple times in the text.
            if only_most_occurance(row):
                return more_occurance(row)

            ## 6-3-2 If the frequency of both words are same(but more than once).
            ...
            ## 6-3-3 If the frequency of every proper nouns in the text is 1.
